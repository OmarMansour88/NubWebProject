...
define([], function() {
    ...
    var objectMetadata;
    var context = {"object" : "RevokeStopPayment", "objectService" : "ChequeManagement"};
    ...
    function RevokeStopPayment(defaultValues) {
        var privateState = {};
        context["field"] = "accountID";
        context["metadata"] = (objectMetadata ? objectMetadata["accountID"] : null);
    ...
    BaseModel.isParentOf(RevokeStopPayment);

    //Create new class level validator object
    BaseModel.Validator.call(RevokeStopPayment);

    var registerValidatorBackup = RevokeStopPayment.registerValidator;

    RevokeStopPayment.registerValidator = function() {
        var propName = arguments[0];
        if(!setterFunctions[propName].changed) {
            var setterBackup = setterFunctions[propName];
            setterFunctions[arguments[0]] = function() {
                if(RevokeStopPayment.isValid(this, propName, val)) {
    ...
    //For Operation 'createRequest' with service id 'revokeStopPaymentRequest5134'
     RevokeStopPayment.createRequest = function(params, onCompletion){
        return RevokeStopPayment.customVerb('createRequest', params, onCompletion);
     };

    var relations = [];

    RevokeStopPayment.relations = relations;

    RevokeStopPayment.prototype.isValid = function() {
        return RevokeStopPayment.isValid(this);
    };

    RevokeStopPayment.prototype.objModelName = "RevokeStopPayment";
    RevokeStopPayment.prototype.objServiceName = "ChequeManagement";
     ...
     */
    RevokeStopPayment.registerProcessors = function(options, successCallback, failureCallback) {
        ...
        kony.mvc.util.ProcessorUtils.getMetadataForObject("ChequeManagement", "RevokeStopPayment", options, metaDataSuccess, metaDataFailure);
    };

    //clone the object provided in argument.
    RevokeStopPayment.clone = function(objectToClone) {
        var clonedObj = new RevokeStopPayment();
