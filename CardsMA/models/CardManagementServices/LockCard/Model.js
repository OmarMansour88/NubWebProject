...
define([], function() {
    ...
    var objectMetadata;
    var context = {"object" : "LockCard", "objectService" : "CardManagementServices"};
    ...
    function LockCard(defaultValues) {
        var privateState = {};
        context["field"] = "accountId";
        context["metadata"] = (objectMetadata ? objectMetadata["accountId"] : null);
    ...
    BaseModel.isParentOf(LockCard);

    //Create new class level validator object
    BaseModel.Validator.call(LockCard);

    var registerValidatorBackup = LockCard.registerValidator;

    LockCard.registerValidator = function() {
        var propName = arguments[0];
        if(!setterFunctions[propName].changed) {
            var setterBackup = setterFunctions[propName];
            setterFunctions[arguments[0]] = function() {
                if(LockCard.isValid(this, propName, val)) {
    ...
    //For Operation 'createRequest' with service id 'createOrder2180'
     LockCard.createRequest = function(params, onCompletion){
        return LockCard.customVerb('createRequest', params, onCompletion);
     };

    var relations = [];

    LockCard.relations = relations;

    LockCard.prototype.isValid = function() {
        return LockCard.isValid(this);
    };

    LockCard.prototype.objModelName = "LockCard";
     ...
     */
    LockCard.registerProcessors = function(options, successCallback, failureCallback) {
        ...
        kony.mvc.util.ProcessorUtils.getMetadataForObject("CardManagementServices", "LockCard", options, metaDataSuccess, metaDataFailure);
    };

    //clone the object provided in argument.
    LockCard.clone = function(objectToClone) {
        var clonedObj = new LockCard();
