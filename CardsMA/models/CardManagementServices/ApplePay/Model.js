...
define([], function() {
    ...
    var objectMetadata;
    var context = {"object" : "ApplePay", "objectService" : "CardManagementServices"};
    ...
    function ApplePay(defaultValues) {
        var privateState = {};
        context["field"] = "errmsg";
        context["metadata"] = (objectMetadata ? objectMetadata["errmsg"] : null);
    ...
    BaseModel.isParentOf(ApplePay);

    //Create new class level validator object
    BaseModel.Validator.call(ApplePay);

    var registerValidatorBackup = ApplePay.registerValidator;

    ApplePay.registerValidator = function() {
        var propName = arguments[0];
        if(!setterFunctions[propName].changed) {
            var setterBackup = setterFunctions[propName];
            setterFunctions[arguments[0]] = function() {
                if(ApplePay.isValid(this, propName, val)) {
    ...
    //For Operation 'AddCard' with service id 'linkCardWithPaymentService7572'
     ApplePay.AddCard = function(params, onCompletion){
        return ApplePay.customVerb('AddCard', params, onCompletion);
     };

    var relations = [];

    ApplePay.relations = relations;

    ApplePay.prototype.isValid = function() {
        return ApplePay.isValid(this);
    };

    ApplePay.prototype.objModelName = "ApplePay";
     ...
     */
    ApplePay.registerProcessors = function(options, successCallback, failureCallback) {
        ...
        kony.mvc.util.ProcessorUtils.getMetadataForObject("CardManagementServices", "ApplePay", options, metaDataSuccess, metaDataFailure);
    };

    //clone the object provided in argument.
    ApplePay.clone = function(objectToClone) {
        var clonedObj = new ApplePay();
