define([ './ParserUtilsManager','./UnifiedTransferDAO','./ValidationUtilManager','FormatUtil','./DataProcessorUtility','JSONValidator','DataValidationFramework/DataValidationHandler'], function (ParserUtilsManager, UnifiedTransferDAO, ValidationUtilManager,FormatUtil,DataProcessorUtility,JSONValidator,DataValidationHandler)  {
  ...
  return {
    ...
    preShow: function() {
      try {
        var scope = this;
		scope.isSingleUser = applicationManager.getUserPreferencesManager().isSingleCustomerProfile;
        scope.inputPool = {}; 
		scope.keypadString = '';
    ...
    **/  
    setAddAddressScreenData: function(){
      try{
        ...
        for(var i=1; i<=8; i++){
          var fieldType = scope.getFieldValue(scope['_addAddressField'+i+"Value"], "fieldType");
          if(fieldType === "phoneNumber"){
            scope.view['txtAddressFieldValue'+i].textInputMode = constants.TEXTBOX_INPUT_MODE_NUMERIC;
          }
          scope.view['lblAddressField'+i].skin = scope.getFieldValue(scope._sknQuestionLbl);
          scope.view['txtAddressFieldValue'+i].skin = scope.getFieldValue(scope._textBoxNormalSkin); 
          scope.view['txtAddressFieldValue'+i].focusSkin = scope.getFieldValue(scope._sknInputBoxBorder);
        }
        //fieldText
    ...
    */
    setAddAddressDefaultText: function(){
      ...
      for(var i=1;i<=6;i++){
        scope.view["lblAddressField"+i].text = scope.getFieldValue(scope['_adsTextBox'+i+"Label"]);
        scope.view["txtAddressFieldValue"+i].placeHolder = scope.getFieldValue(scope['_adsTextBox'+i+"Value"], "placeHolder");
      }
    ...
    */
    ...
    setAddAddressData: function(){
      var scope = this;
      for(var i=1; i<=6; i++){
        var texboxVisibility = scope['_textBox'+i+"Visibility"];
        if(texboxVisibility === false){
          scope.view['lblAddressField'+i].setVisibility(false);
          scope.view['flxAddressFieldValue'+i].setVisibility(false);
        }
      }
      for(var i=1;i<=6;i++){
        scope.view["txtAddressFieldValue"+i].text = scope.getFieldValue(scope['_adsTextBox'+i+"Value"], "value");
      }
      var addAddress={
        "txtBoxAddressLine01":  scope.view.txtAddressFieldValue1.text,
        "txtBoxAddressLine02":  scope.view.txtAddressFieldValue2.text,
    ...
    onAddAddressContinue: function(){
      try {
        var scope = this;    
        for(var i=1; i<=6; i++)
          if(!kony.sdk.isNullOrUndefined((scope.view['txtAddressFieldValue'+i].text)) && scope.view['txtAddressFieldValue'+i].text !== "") {
            scope.inputPool[scope.getFieldValue(scope['_adsTextBox'+i+"Value"],"inputPoolKey")] = scope.view['txtAddressFieldValue'+i].text;
          }
        scope.setVerifyDetails();
        scope.navigateTo("flxVerifyDetails", "flxVfsHeaderTop", scope.getFieldValue(scope._vfsSectionTitle));
      } catch(err) {
    ...
    */
    setAccountNumberSkins: function(){
      ...
      for(var i=1; i<=12; i++){
        scope.view['lblDigit'+i].skin = scope.getFieldValue(scope._accInputFieldMaskedSkin);
      }
    ...
    */
    setReEnterAccountNumberSkins: function(){
      ...
      for(var i=1; i<=12; i++){
        scope.view['lblDigit0'+i].skin = scope.getFieldValue(scope._accInputFieldMaskedSkin);
      }
   ...
   * Component setPayeeNicknameActions
    ...
    setPayeeNicknameActions : function()
    {
      try{
        ...
        scope.view.txtNickName.onTextChange = scope.setPayeeNicknameData.bind(this);
      }
      catch(err) {
        var errObj = {
