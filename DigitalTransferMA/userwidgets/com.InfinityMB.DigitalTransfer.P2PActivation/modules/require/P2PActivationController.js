define(['./P2PActivationBusinessController','./P2PActivationStore', 'DataValidationFramework/DataValidationHandler'], function(BusinessController, P2PActivationStore, DataValidationHandler) {
  ...
  return {
	...
	*/
    storeContextData : function() {
      ...
      try {
        ...
        userData[scope.getMappedValueForWidget("field2Value", scope._dataMapping)]   = "Phone Number";
        ...
        userData["selectedDefaultAccount"] = (scope.context.hasOwnProperty("default_to_account_p2p") && !kony.sdk.isNullOrUndefined(scope.context["default_to_account_p2p"])) ? scope.context["default_to_account_p2p"] : "Add";
        userData[scope.getMappedValueForWidget("field4Value", scope._dataMapping)]    = scope.context.hasOwnProperty("formattedDepositAccount") ? scope.context["formattedDepositAccount"] :  "Add";
        userData[scope.getMappedValueForWidget("field5Value", scope._dataMapping)]    = "Add";
    ...
    render : function () {
      ...
      if(scope.collectionObj.Collection.hasOwnProperty("ServiceAcknowledgement")) {
       ...
       if(scope.collectionObj.Collection.hasOwnProperty("deactivateP2PAcknowledgement")) {
        ...
        if(scope.businessController.getDataBasedOnDataMapping("tbxTransferLimit", scope._dataMapping) === "Add") {
	...
	*/
    getUserCollectionData :  function(collectionData) {
      ...
      if(currentContactType === "Phone Number") {
  ...
  */
    setActivateScreenValue : function() {
      ...
      if(scope.view.lblField5Value.text !== "Add" && scope.view.lblField4Value.text !== "Add") {
  ...
  */
    setContactTypeFieldValues : function() {
      ...
      if(scope.contactList.length > 1 || currentContactType === "National ID") {
   ...
   */
    ...
    UIMapping : function(ResponseArray, segDataJSON, key){
      ...
      ResponseArray.map(function(record){
        ...
        for(key in segRecord) {
          if(key === "imgUserIcon") {
            segRecord[key] = (!kony.sdk.isNullOrUndefined(record["isBusinessAccount"]) && record["isBusinessAccount"] === "true") ? "businessaccount.png" : "personalaccount.png";
	...
	*/
    setConfirmDetailsActions : function() {
      ...
      scope.view.flxField3.onClick = function() {
        if((currentContactType === "Phone Number" || currentContactType === "Email")) {
          if(scope.contactList.length > 1) {
            scope.view.lblPhnoSubHeader.text = (currentContactType === "Phone Number") ? scope.businessController.getDataBasedOnDataMapping("lblPreferredMobile", scope._dataMapping) : scope.businessController.getDataBasedOnDataMapping("lblPreferredEmail", scope._dataMapping);
    ...
    setContactListDatainSegment : function() {
      ...
      for (var each in ContactList) {
        if(scope.isEmptyNullUndefined(ContactList[each].Description)) {
          ContactList[each].Description = currentContactType === "Phone Number" ? "Alternate Mobile" : "Alternate Email";
        }
        var segmentData = {
          ...
          "lblField2" :  currentContactType === "Phone Number" ? scope.formatPhoneNumber(ContactList[each].Value) : ContactList[each].Value,
	...
	*/
    setActivateP2PGuideliness : function() {
      var scope = this; 
      try {
      scope.view.imgCheckbox.src = "inactive_checkbox.png";
	...
	*/
    ...
    onTermsandConditionsAgree : function() {
      var scope = this;
      try {
        if(scope.view.imgCheckbox.src === "inactive_checkbox.png") {
          scope.view.imgCheckbox.src = "checkbox.png";
          scope.enableButton("btnActivateP2PContinue");
        } else if(scope.view.imgCheckbox.src === "checkbox.png") {
          scope.view.imgCheckbox.src = "inactive_checkbox.png";
          scope.disableButton("btnActivateP2PContinue");
        }
      } catch(err) {
    ...
    onPrimaryContactSelection : function() {
      ...
      scope.navigateTo("flxConfirmDetails", scope.businessController.getDataBasedOnDataMapping("lblActivateP2P", scope._dataMapping));
      if(scope.view.lblField2Value.text === "Phone Number") {
    ...
    setTransferLimitActions : function() {
      ...
      if(!kony.sdk.isNullOrUndefined(transferLimit) && transferLimit !== "" && transferLimit !== "Add") {
   ...
   onCurrencySelection : function() {
     ...
     try {
      ...
      scope.navigateTo("flxTransferLimit", scope.businessController.getDataBasedOnDataMapping("lblTransferLimit", scope._dataMapping));
      scope.businessController.updateDataInCollection("Add","tbxTransferLimit", scope._dataMapping);
    ...
    setTransferLimitTextBoxValue : function() {
      ...
      var serviceParamters = scope.serviceParameters;
      var transferLimit = scope.businessController.getDataBasedOnDataMapping("tbxTransferLimit", scope._dataMapping);
      if(!scope.isEmptyNullUndefined(scope.businessController.getDataBasedOnDataMapping("tbxCurrency", scope._dataMapping)) && transferLimit === "Add") {
        scope.view.lblCurrencyDropdown.text = scope.businessController.getDataBasedOnDataMapping("tbxCurrency", scope._dataMapping);
        scope.view.lblDollar.text = "0.00";
        scope.keypadString = "0.00";
        scope.disableButton("btnAmountContinue");
      }
      if(transferLimit !== "Add") {
