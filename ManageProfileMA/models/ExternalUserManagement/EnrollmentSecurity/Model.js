...
define([], function() {
    ...
    var objectMetadata;
    var context = {"object" : "EnrollmentSecurity", "objectService" : "ExternalUserManagement"};
    ...
    function EnrollmentSecurity(defaultValues) {
        var privateState = {};
        context["field"] = "id";
        context["metadata"] = (objectMetadata ? objectMetadata["id"] : null);
    ...
    BaseModel.isParentOf(EnrollmentSecurity);

    //Create new class level validator object
    BaseModel.Validator.call(EnrollmentSecurity);

    var registerValidatorBackup = EnrollmentSecurity.registerValidator;

    EnrollmentSecurity.registerValidator = function() {
        var propName = arguments[0];
        if(!setterFunctions[propName].changed) {
            var setterBackup = setterFunctions[propName];
            setterFunctions[arguments[0]] = function() {
                if(EnrollmentSecurity.isValid(this, propName, val)) {
    ...
    //For Operation 'generateCaptchaForEnrollment' with service id 'GenerateCaptchaForEnrollment7181'
     EnrollmentSecurity.generateCaptchaForEnrollment = function(params, onCompletion){
        return EnrollmentSecurity.customVerb('generateCaptchaForEnrollment', params, onCompletion);
     };

    var relations = [];

    EnrollmentSecurity.relations = relations;

    EnrollmentSecurity.prototype.isValid = function() {
        return EnrollmentSecurity.isValid(this);
    };

    EnrollmentSecurity.prototype.objModelName = "EnrollmentSecurity";
     ...
     */
    EnrollmentSecurity.registerProcessors = function(options, successCallback, failureCallback) {
        ...
        kony.mvc.util.ProcessorUtils.getMetadataForObject("ExternalUserManagement", "EnrollmentSecurity", options, metaDataSuccess, metaDataFailure);
    };

    //clone the object provided in argument.
    EnrollmentSecurity.clone = function(objectToClone) {
        var clonedObj = new EnrollmentSecurity();
        clonedObj.fromJsonInternal(objectToClone.toJsonInternal());
        return clonedObj;
    };

    return EnrollmentSecurity;
});