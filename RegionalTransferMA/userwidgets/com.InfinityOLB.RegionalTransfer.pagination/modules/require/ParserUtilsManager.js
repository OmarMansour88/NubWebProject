define(function () {
  ...
  /**
   * Utility file used for parsing the component supported text formats and provides
     helper methods for context and service response related data.
   */
   ...
   * @param {Object} context
   */
  ParserUtilsManager.prototype.setTokens = function(tokenJSON){
    for(var key in tokenJSON){
      this.tokens[key] = tokenJSON[key];
    }
  };
  ...
  /**
   * Clears token object to ParserUtilityManager.
   */
  ParserUtilsManager.prototype.clearTokens = function(){
    this.tokens = {};
  };
  ...
  ParserUtilsManager.prototype.getDecoratorText = function(decoratorText){
    if(decoratorText.indexOf("{i.") != -1)
      ...
      if(unicode == "c"){
        ...
        return this.getTokenValue(decoratorText.substring(valueStartIndex,decoratorText.length-1));
      }
      else{
  ...
  /**
   * get token value for the String.
   *
   * @param {String} text
   * 
   * @return {String} - token value for string
   */
  ParserUtilsManager.prototype.getTokenValue = function(text){
    var tokenValue = "";
    try{
      tokenValue = (this.tokens[text] || this.tokens[text] == 0) ? this.tokens[text] : "";
    }catch(e){
      kony.print(e);
    }
    if(tokenValue==="")
