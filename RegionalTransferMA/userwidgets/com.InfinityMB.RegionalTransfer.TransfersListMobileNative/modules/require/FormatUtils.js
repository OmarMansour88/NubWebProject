  ...
  FormatUtils.prototype.formatText = function(text,type,formatSkins,FormatvalueJSON,currencyCode){
    ...
    if(fieldValue){
      if(fieldType == "Amount"){
       ...
       else if(fieldType == "AccountNumberWithNickname" || fieldType == "AccountNumberWithNicknameAndIcon"){
        var accNo = fieldValue.accountNumber;
        var lastdigit = accNo.slice(-(JSON.parse(FormatJSON.accountLength)));
        var formatAccNameAndNumber = {};                            
       // formatData.text = this.formatAccountNumber(JSON.parse(formatUtils.ACCOUNT_NUMBER_FORMAT), lastdigit);
        formatAccNameAndNumber.skin = formatSkins.ACCOUNT_NUMBER_SKIN ? formatSkins.ACCOUNT_NUMBER_SKIN: formatSkins.TEXT_SKIN;
        if(fieldValue.nickName){
           formatAccNameAndNumber.text = fieldValue.nickName +"..."+lastdigit;
        }
        else if (fieldValue.name) {
          formatAccNameAndNumber.text = fieldValue.name +"..."+lastdigit;
        }
        else {
          formatAccNameAndNumber.text = "..." + lastdigit;
        }
        return formatAccNameAndNumber;
      } 
      else {
