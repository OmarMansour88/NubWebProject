...
define([], function() {
    ...
    var objectMetadata;
    var context = {"object" : "CustomView", "objectService" : "Holdings"};
    ...
    function CustomView(defaultValues) {
        var privateState = {};
        context["field"] = "errmsg";
        context["metadata"] = (objectMetadata ? objectMetadata["errmsg"] : null);
    ...
    BaseModel.isParentOf(CustomView);

    //Create new class level validator object
    BaseModel.Validator.call(CustomView);

    var registerValidatorBackup = CustomView.registerValidator;

    CustomView.registerValidator = function() {
        var propName = arguments[0];
        if(!setterFunctions[propName].changed) {
            var setterBackup = setterFunctions[propName];
            setterFunctions[arguments[0]] = function() {
                if(CustomView.isValid(this, propName, val)) {
    ...
    //For Operation 'updateView' with service id 'UpdateCustomView6488'
     CustomView.updateView = function(params, onCompletion){
        return CustomView.customVerb('updateView', params, onCompletion);
     };

    //For Operation 'createView' with service id 'CreateCustomView1406'
     CustomView.createView = function(params, onCompletion){
        return CustomView.customVerb('createView', params, onCompletion);
     };

    //For Operation 'getView' with service id 'GetCustomView2376'
     CustomView.getView = function(params, onCompletion){
        return CustomView.customVerb('getView', params, onCompletion);
     };

    //For Operation 'deleteView' with service id 'DeleteCustomView3490'
     CustomView.deleteView = function(params, onCompletion){
        return CustomView.customVerb('deleteView', params, onCompletion);
     };

    var relations = [];

    CustomView.relations = relations;

    CustomView.prototype.isValid = function() {
        return CustomView.isValid(this);
    };

    CustomView.prototype.objModelName = "CustomView";
     ...
     */
    CustomView.registerProcessors = function(options, successCallback, failureCallback) {
        ...
        kony.mvc.util.ProcessorUtils.getMetadataForObject("Holdings", "CustomView", options, metaDataSuccess, metaDataFailure);
    };

    //clone the object provided in argument.
    CustomView.clone = function(objectToClone) {
        var clonedObj = new CustomView();
