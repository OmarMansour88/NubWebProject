define(['FormatUtil'],function (formatingUtil) {
  ...
  /**
  setting europe demography 
  */
     FormatUtils.prototype.setEuropeFlow =  function(euroflow) {
       this.euro = euroflow;
     },
    ...
    FormatUtils.prototype.formatText = function(text, type, formatSkins, FormatvalueJSON, currencyCode) {
        ...
        if (fieldValue) {
            if (fieldType == "Amount") {
              ...
              return this.formatAmountandAppendCurrencySymbol(deformattedAmt, FormatJSON.currencyCode, FormatJSON.amountFormat, formatSkins);
                ...
                return formatDate;
            } else if (fieldType == "accountDateFormat" && FormatJSON.dateFormat) {
              var formattedDate = fieldValue.slice(0,4)+"-"+fieldValue.slice(4,6)+"-"+fieldValue.slice(6,8);
                var dateObjFromString = this.getDateObjectfromString(formattedDate)
                var formatDate = {};
                formatDate.skin = formatSkins.DATE_SKIN ? formatSkins.DATE_SKIN : formatSkins.TEXT_SKIN;
                formatDate.text = this.getFormatedDateString(dateObjFromString, FormatJSON.dateFormat);
                return formatDate;
              ...
              var val  =formatPercentage.text;
              if(val.slice(0, 1) == "-"){
                formatPercentage.skin = formatSkins.NEGATIVE_PERCENTAGE_SKIN;
                return formatPercentage;
              }
              else {
                formatPercentage.skin = formatSkins.POSITIVE_PERCENTAGE_SKIN;
                formatPercentage.text = "+" + formatPercentage.text;
                return formatPercentage;
              }
              	
            }
              else if (fieldType == "PL") {
                var formatPercentage = {};
                formatPercentage = this.formatAmountandAppendCurrencySymbol(fieldValue, currencyCode, FormatJSON.amountFormat, formatSkins);
              var val  = formatPercentage.text;
              if(fieldValue.slice(0, 1) == "-"){
                 formatPercentage.skin = formatSkins.NEGATIVE_PERCENTAGE_SKIN;
              }
               else{
                 formatPercentage.text = "+" + val;
                  formatPercentage.skin = formatSkins.POSITIVE_PERCENTAGE_SKIN;
               }
                return formatPercentage;
            } 
            ...
            var formattedText = {};
            var fullLen = fieldValue.length;
            if (fullLen>20){
              formattedText.text = fieldValue.slice(0, 19) + "..."; 
            }
            else
              formattedText.text = fieldValue;
             formattedText.skin = formatSkins.TEXT_SKIN;
          } else if (fieldType == "Hidden"){
            return {
                text: fieldValue,
                skin: formatSkins.TEXT_HIDDEN
            };
          }
          else{
              return {
