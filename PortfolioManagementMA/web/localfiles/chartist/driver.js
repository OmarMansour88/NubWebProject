    ...
    switch (filter) {
        case '1D': interpolationFunc = (value, index, labels) => {
            ...
            if (labels.length <= 5) {
                ...
                let initialdiv;
                if (newResponse.length <=6) {
                    initialdiv = 1;
                } else if (newResponse.length > 6 && newResponse.length <= 12) {
					initialdiv = 2;
                } else if (newResponse.length > 12 && newResponse.length <= 16) {
					initialdiv = 3;
                } else if (newResponse.length > 16 && newResponse.length <= 22) {
					initialdiv = 4;
                } else if (newResponse.length > 22 && newResponse.length <= 29) {
					initialdiv = 5;
                } else {
					initialdiv = Math.floor(newResponse.length / 5);
                }

                //Adding the X-Axis Values maximum of 5
                let div = initialdiv;
                let xcount = 1;
            ...
            interpolationFunc = (value, index, labels) => {
                try {
                    ...
                    if (index === labels.findIndex(l => l === value) && expectedMonth.includes(value.slice(0, 3)) &&
                       ...
                       (value.slice(4, 8) == d.getFullYear() && value.slice(0, 3) === "Jan")) {
