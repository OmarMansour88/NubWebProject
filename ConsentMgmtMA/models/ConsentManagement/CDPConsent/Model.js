...
define([], function() {
    ...
    var objectMetadata;
    var context = {"object" : "CDPConsent", "objectService" : "ConsentManagement"};
    ...
    function CDPConsent(defaultValues) {
        var privateState = {};
        context["field"] = "id";
        context["metadata"] = (objectMetadata ? objectMetadata["id"] : null);
    ...
    BaseModel.isParentOf(CDPConsent);

    //Create new class level validator object
    BaseModel.Validator.call(CDPConsent);

    var registerValidatorBackup = CDPConsent.registerValidator;

    CDPConsent.registerValidator = function() {
        var propName = arguments[0];
        if(!setterFunctions[propName].changed) {
            var setterBackup = setterFunctions[propName];
            setterFunctions[arguments[0]] = function() {
                if(CDPConsent.isValid(this, propName, val)) {
    ...
    //For Operation 'getConsent' with service id 'getCDPConsentGateway3152'
     CDPConsent.getConsent = function(params, onCompletion){
        return CDPConsent.customVerb('getConsent', params, onCompletion);
     };

    //For Operation 'updateConsent' with service id 'createOrder3271'
     CDPConsent.updateConsent = function(params, onCompletion){
        return CDPConsent.customVerb('updateConsent', params, onCompletion);
     };

    var relations = [];

    CDPConsent.relations = relations;

    CDPConsent.prototype.isValid = function() {
        return CDPConsent.isValid(this);
    };

    CDPConsent.prototype.objModelName = "CDPConsent";
     ...
     */
    CDPConsent.registerProcessors = function(options, successCallback, failureCallback) {
        ...
        kony.mvc.util.ProcessorUtils.getMetadataForObject("ConsentManagement", "CDPConsent", options, metaDataSuccess, metaDataFailure);
    };

    //clone the object provided in argument.
    CDPConsent.clone = function(objectToClone) {
        var clonedObj = new CDPConsent();
